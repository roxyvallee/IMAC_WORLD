...
namespace glimac{
    ...
    double RadialBasicFunction::distance(glm::vec3 A, glm::vec3 B)
    {
        return sqrt( std::pow(A[0]-B[0], 2) + std::pow(A[1]-B[1], 2) + std::pow(A[2]-B[2], 2));
    }

    double RadialBasicFunction::radialFunctionGauss( glm::vec3 A, glm::vec3 B)
    {
    ...
    Eigen::VectorXd RadialBasicFunction::findOmega()
    {
    ...
    RadialBasicFunction::RadialBasicFunction()
    ...
    double RadialBasicFunction::calculBasicFunction(glm::vec3 V)
    {
        double sum = 0.0;
